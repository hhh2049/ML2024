...
class ANN:  # 人工神经网络（分类与回归）算法实现：小批量随机梯度下降法
    def __init__(self, X, y, is_classify=True, hidden_layer_sizes=(10, ),
                 activation="logistic", solver='mbgd', alpha=0.0001,
                 batch_size=5, learning_rate_init=0.001, max_iter=200):
        ...
        self.solver             = solver                   # 优化算法，实现MBGD
        self.alpha              = alpha                    # L2正则项参数（未使用）
        self.batch_size         = batch_size               # MBGD的块大小
    ...
    def build_para(self, hidden_layer_sizes, is_classify): # 构建和初始化参数
        ...
        for i in range(len(hidden_layer_sizes)+1):         # 遍历神经网络的各层，+1为输出层
            if i == 0:                                     # 如果是第一层
            ...
            W = np.random.randn(last, current) * W_width   # 随机生成当前权重矩阵初始值
            self.W_list.append(W)                          # 将当前权重矩阵加入列表
            grad = np.zeros((last, current))               # 生成梯度矩阵
    ...
    def forward_propagation(self, x): # 正向传播
        for i, W in enumerate(self.W_list):                # 遍历神经网络的各层
            if i > 0:                                      # 如果不是第一层
                x = self.out_list[i-1]                     # 获取当前层的输入值
            self.in_list[i] = np.dot(x, W)                 # 计算当前层的输入值向量
            self.out_list[i] = self.f(self.in_list[i])     # 计算当前层的输出值向量

        if self.is_classify:                               # 若用于分类问题
            z = self.in_list[-1]                           # 获取最后一层输入值向量
            self.out_list[-1] = np.exp(z)/np.sum(np.exp(z),
                                    axis=1, keepdims=True) # 按行计算Softmax函数值
            return np.argmax(self.out_list[-1], axis=1)    # 返回分类预测值
        else:                                              # 若用于回归问题
            self.out_list[-1] = self.in_list[-1]           # 输入即输出
            z = self.out_list[-1]                          # 为简化下一行代码
            return z.reshape(len(z), )                     # 返回回归预测值
    ...
    def backward_propagation(self, x, y):  # 误差反向传播
        ...
        for i in range(-1, -layer_count-1, -1):            # 从后往前遍历
            if i == -1:                                    # 如果是最后一层
                ...
                error = np.dot(delta, self.W_list[i+1].T)  # 计算W、δ乘积
            d = self.d                                     # 为缩短下一行代码长度
            self.delta_list[i] = error*d(self.in_list[i])  # 计算当前层δ值

            if i == -layer_count:                          # 如果是第一隐藏层
                zin = x                                    # 获取本层输入，即x
            else:                                          # 如果不是第一隐藏层
                zin = self.out_list[i-1]                   # 获取本层输入
            grad = np.dot(zin.T, self.delta_list[i])       # 计算梯度
            self.grad_list[i] = grad / self.batch_size     # 保存梯度
    ...
    def fit(self):  # 拟合数据，训练模型
        ...
        for _ in range(self.max_iter):                     # 遍历训练轮数
            count = len(self.X) // self.batch_size         # 计算本轮训练次数
            for i in range(count):                         # 遍历训练数据
                start = i * self.batch_size                # 每次训练的起始下标
                end   = (i+1) * self.batch_size            # 每次训练的终止下标
                self.forward_propagation(X[start:end])     # 前向传播
                self.backward_propagation(X[start:end],
                                          Y[start:end])    # 误差反向传播
                self.update_model_parameters()             # 更新各层权重参数

    def predict(self, X):  # 预测一个数据集
        ones = np.ones(len(X))                             # 生成一个值全为1的向量
        X = np.c_[X, ones]                                 # 在每个数据后面添加一个1
        y_hat = self.forward_propagation(X)                # 预测数据集X
