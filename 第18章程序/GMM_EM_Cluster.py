...
class GaussianMixture:  # 使用EM算法求解高斯混合模型
    def __init__(self, X, y, n_components, tol=0.0005, reg_covar=1e-6, max_iter=100):
        ...
        self.m, self.n  = X.shape                            # 训练数据的样本量和特征数
        ...
        self.init_w     = None                               # 用于存储初始参数w
        self.init_Mu    = None                               # 用于存储初始参数Mu
        self.init_Sigma = None                               # 用于存储初始参数Sigma

    def initialize_gauss_model(self):  # 初始化各个高斯分布的权重和参数
        model = K_Means(self.X, n_clusters=self.K)           # 定义K均值类对象
        model.fit()                                          # 执行训练
        self.Mu = model.centers_.copy()                      # K均值簇中心作为初始值

        for i in range(self.K):                              # 遍历各个高斯分布
            self.w[i]     = 1.0 / self.K                     # 初始时各个高斯分量权重相等
            self.Sigma[i] = np.eye(self.n)                   # 协方差矩阵初始化为单位矩阵

        self.init_w     = self.w.copy()                      # 保存初始参数w
        self.init_Mu    = self.Mu.copy()                     # 保存初始参数Mu
        self.init_Sigma = self.Sigma.copy()                  # 保存初始参数Sigma
    ...
    def execute_E(self):  # 执行EM算法的E步
        for i in range(self.m):                              # 遍历所有数据
            ...
            p_xi  = 0.0                                      # 存放p(xi)
            for j in range(self.K):                          # 遍历所有类别（各个高斯分布）
    ...
    def execute_M(self):  # 执行EM算法的M步
        for j in range(self.K):                              # 遍历各个高斯分布
            ...
            sum_qij_xi = np.zeros((self.n, ))                # 存放式（16.68）均值的分子
            for i in range(self.m):                          # 遍历所有数据
                sum_qij_xi += self.q[i, j] * self.X[i]       # 计算式（16.68）均值的分子
            self.Mu[j] = sum_qij_xi / sum_qij                # 计算并更新各个高斯分布的均值

            sum_sigma = np.zeros((self.n, self.n))           # 存放式（16.70）协方差的分子
            for i in range(self.m):                          # 遍历所有数据
                xi, muj = self.X[i], self.Mu[j]              # 为缩短下一行代码的长度
                product = np.outer(xi - muj, xi - muj)       # 计算矩阵乘积
                sum_sigma += self.q[i, j] * product          # 计算协（16.70）方差式的分子
