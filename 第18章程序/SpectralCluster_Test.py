...
def main():
    ...
    gamma = [50, 25, 0.5, 5.0, 1.0, 2.0]                       # gamma参数列表
    for i, (data, cluster) in enumerate(data_cluster_list):    # 遍历所有数据集（共6个）
        our_model = SpectralCluster(data, n_clusters=cluster,
                                    gamma=gamma[i])            # 生成自实现的谱聚类对象
        our_model.fit()                                        # 执行训练
        our_label_list.append(our_model.labels_)               # 保存簇标签

        skl_model = SpectralClustering(n_clusters=cluster,
                                       gamma=gamma[i])         # 生成官方库的谱聚类对象
    ...
    for i, label in enumerate(label_list):                     # 遍历自实现和官方库的簇标签
        ...
        for j in range(len(data_cluster_list)):                # 可视化聚类后形成的簇
            ...
            if i == 0:                                         # 如果是绘制自实现散点图
                plt.title("Our SpectralCluster on Dataset %d" % (j+1))  # 设置标题
            else:                                              # 如果是绘制官方库散点图
                plt.title("SKL SpectralCluster on Dataset %d" % (j+1))  # 设置标题
